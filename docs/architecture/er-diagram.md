...
| Auditoria | Eventos e correlação em CloudWatch + trilha MySQL `OrdemServicoStatusHistorico` — ADR-010 |
...
erDiagram
  ...
  OrdemServico ||--o{ OrdemServicoStatusHistorico : registra
...
| OrdemServico → OrdemServicoStatusHistorico | **1:N** | Trilha de status (ADR-010); base do tempo médio por fase Diagnóstico/Execução/Finalização. |
...
Fluxo típico de abertura: validar Cliente + Veiculo → criar `OrdemServico` com `status = RECEBIDA` → gravar itens → persistir `total` → linha em `OrdemServicoStatusHistorico`. Transições posteriores (diagnóstico → execução → finalização) atualizam o enum `StatusOs` e acrescentam histórico sem quebrar FKs.
